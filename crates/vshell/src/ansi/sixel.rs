...
use crate::{ansi::graphics::MAX_GRAPHIC_DIMENSIONS, config::colors::ColorRgb};
...
impl Parser {
    ...
    pub fn new(
        params: &Params,
        shared_palette: Option<Vec<ColorRgb>>,
    ) -> Parser {
    ...
    fn ensure_size(
        ...
        if width > MAX_GRAPHIC_DIMENSIONS[0]
            || height > MAX_GRAPHIC_DIMENSIONS[1]
        {
    ...
    pub fn finish(
        mut self,
    ) -> Result<(SugarGraphicData, Vec<ColorRgb>), Error> {
        self.finish_command()?;
...
mod tests {
    ...
    fn parse_command_parameters() {
        let mut command_parser =
            CommandParser::new(SixelCommand::ColorIntroducer);
    ...
    fn resize_picture() -> Result<(), Error> {
        ...
        for (n, row) in parser.pixels.chunks(WIDTH + 5).enumerate() {
            if n < HEIGHT {
                ...
                assert!(row[WIDTH..]
                    .iter()
                    .all(|pixel| *pixel == REG_TRANSPARENT));
            } else {
    ...
    fn load_sixel_files() {
        let images_dir =
            Path::new(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/sixel"));
        ...
        for test_image in &test_images {
            ...
            if let Some(pos) =
                sixel.iter().position(|&b| b == 0x1B || b == 0x9C)
            {
                sixel.truncate(pos);
            }
