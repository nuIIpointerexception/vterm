...
impl Label {
    pub fn new(font: &Font, content: &str) -> Self {
        ...
        let text_width = lines
            .iter()
            .map(|line| font.calculate_text_bounds(line).width())
        ...
        Self {
            ...
            bg: Rectangle::new(
                ...
                CompositeStyle::default()
                    .with_background(FillStyle::Color(Color::new(1.0, 1.0, 1.0, 0.1))),
            ),
        }
    }
...
impl<Message> Widget<Message> for Label {
    ...
    fn draw_frame(&mut self, _internal_state: &mut InternalState, frame: &mut Frame) -> Result<()> {
        let bg_top_left = Vec2::new(
            self.bounds.top_left.x + self.text_size.x / 2.0,
            self.bounds.top_left.y + self.text_size.y / 2.0,
        );
