...
impl MSAARenderPass {
    pub(super) fn create_msaa_render_target(
        ...
        let samples = Self::pick_max_supported_msaa_count(&vk_dev, vk::SampleCountFlags::TYPE_4);
        let (swap_extent, format) = vk_dev.with_swapchain(|swap| (swap.extent, swap.format));
        ...
        let view =
            Arc::new(ImageView::new_2d(msaa_render_target, format, vk::ImageAspectFlags::COLOR)?);
        Ok(view)
    }
    ...
    fn pick_max_supported_msaa_count(
        ...
        let props =
            unsafe { vk_dev.instance.ash.get_physical_device_properties(vk_dev.physical_device) };
